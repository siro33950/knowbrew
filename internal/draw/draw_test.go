...
func TestEnsureRepositorySubjectDoesNotCreateFromNonRepositoryCWD(t *testing.T) {
	...
	added, warnings, err := ensureRepositorySubject(context.Background(), dataStore, &candidate)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEnsureRepositorySubjectCreatesMasterWithoutAssigningFeedstock(t *testing.T) {
	...
	added, _, err := ensureRepositorySubject(context.Background(), dataStore, &candidate)
	if err != nil {
		t.Fatal(err)
	}
	if added != 1 {
...
`
	...
	summaryText, warnings, err := summaryPrompt(cfg, dataStore, target.ID)
	if err != nil {
		t.Fatal(err)
	}
	if len(warnings) != 0 {
	...
	assertionText, warnings, err := annotationPrompt(cfg, dataStore, target.ID, feedstocks)
	if err != nil {
		t.Fatal(err)
	}
	if len(warnings) != 0 {
...
func TestAnnotationPromptIncludesOnlyThreePriorTurnsWithinSession(t *testing.T) {
	...
	prompt, _, err := annotationPrompt(cfg, dataStore, target.ID, feedstocks, snapshots)
	if err != nil {
		t.Fatal(err)
	}
	for _, required := range []string{
	...
	prompt, _, err = annotationPrompt(withoutContext, dataStore, target.ID, feedstocks, snapshots)
	if err != nil {
		t.Fatal(err)
	}
	for _, forbidden := range []string{
