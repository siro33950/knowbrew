...
func (tx *Transaction) StageBrewedFeedstock(feedstock domain.Feedstock, when time.Time) error {
	...
	current.Assertions = append([]domain.Assertion(nil), feedstock.Assertions...)
	current.BrewedAssertions = domain.UniqueSorted(feedstock.BrewedAssertions)
	current.Assertions, current.Types, err = tx.store.normalizeAssertions(current.Assertions)
	if err != nil {
		return fmt.Errorf("feedstock assertions: %w", err)
	}
	current.Subjects = subjectsFromAssertions(current.Assertions)
	processed := make(map[string]struct{}, len(current.BrewedAssertions))
	for _, assertionID := range current.BrewedAssertions {
	...
	if complete && (len(current.Assertions) == 0 || len(current.BrewedAssertions) > 0) {
		if current.BrewedAt == nil {
			current.BrewedAt = &when
		}
	} else {
		current.BrewedAt = nil
	}
	if err := domain.ValidateFeedstock(current); err != nil {
		return err
	}
