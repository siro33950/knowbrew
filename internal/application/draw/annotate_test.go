...
func TestAnnotateRejectsAlreadyAnnotatedFeedstock(t *testing.T) {
	...
	if _, err := annotateForTest(context.Background(), dataStore, annotation); err != nil {
		t.Fatal(err)
	}
	if _, err := annotateForTest(context.Background(), dataStore, annotation); err == nil ||
...
func TestSummaryAndAnnotationUseSeparateStateTransitions(t *testing.T) {
	...
	if err := summarizeForTest(context.Background(), dataStore, feedstock.ID, "replacement"); err == nil ||
